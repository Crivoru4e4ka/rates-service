...
- TTL-кэш ответов провайдера в памяти (`PROVIDER_CACHE_TTL`): повторные запросы
  одной пары не расходуют квоту внешнего API; hit/miss видны в `/metrics`
...
**TTL-кэш ответов провайдера.** Повторные POST одной пары сжигали квоту
...
`FetchedAt` — время реального получения данных, без «омоложения».

**Redis сознательно не используется.** Для single-instance тот же эффект даёт
кэш в памяти — без новой инфраструктуры и точки отказа. Redis становится
правильным решением при любом из условий: кэш должен быть общим между
несколькими инстансами; переживать рестарты при дорогом API; нужен
распределённый rate-limiter. Остальное тоже сознательно не добавлено:
брокеры сообщений, gRPC, OpenTelemetry — на этом масштабе они решают
